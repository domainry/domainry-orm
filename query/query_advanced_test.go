...
func TestSelectRowLocking(t *testing.T) {
	tests := []struct {
		...
		{"for update", func(r dialect.Renderer) (string, []any, error) {
			return query.NewSelectBuilder(r, "jobs").Columns("id").Where(query.Equal("state", "queued")).ForUpdate().Build()
		}, `SELECT "id" FROM "jobs" WHERE "state" = ? FOR UPDATE`, []any{"queued"}},
		{"for update skip locked", func(r dialect.Renderer) (string, []any, error) {
			return query.NewSelectBuilder(r, "jobs").Columns("id").Where(query.Equal("state", "queued")).ForUpdate("SKIP LOCKED").Build()
		...
		{"for update nowait", func(r dialect.Renderer) (string, []any, error) {
			return query.NewSelectBuilder(r, "jobs").Columns("id").Where(query.Equal("id", 1)).ForUpdate("NOWAIT").Build()
		}, `SELECT "id" FROM "jobs" WHERE "id" = ? FOR UPDATE NOWAIT`, []any{1}},
		{"for share", func(r dialect.Renderer) (string, []any, error) {
			return query.NewSelectBuilder(r, "jobs").Columns("id").Where(query.Equal("id", 1)).ForShare().Build()
		}, `SELECT "id" FROM "jobs" WHERE "id" = ? FOR SHARE`, []any{1}},
		{"for no key update", func(r dialect.Renderer) (string, []any, error) {
			return query.NewSelectBuilder(r, "jobs").Columns("id").Where(query.Equal("id", 1)).ForNoKeyUpdate().Build()
		}, `SELECT "id" FROM "jobs" WHERE "id" = ? FOR NO KEY UPDATE`, []any{1}},
		{"for key share", func(r dialect.Renderer) (string, []any, error) {
			return query.NewSelectBuilder(r, "jobs").Columns("id").Where(query.Equal("id", 1)).ForKeyShare().Build()
...
func TestSelectSetOperations(t *testing.T) {
	base := func(r dialect.Renderer, table string, value any) *query.SelectBuilder {
		return query.NewSelectBuilder(r, table).Columns("id").Where(query.Equal("kind", value))
	}
	...
	t.Run("union all with order limit", func(t *testing.T) {
		q := base(sqliteRenderer(t), "a", "x").
			...
			OrderBy(query.Ascending("id")).Limit(10)
		sql, args, err := q.Build()
		assertSQL(t, sql, args, err,
...
func TestSelectCTE(t *testing.T) {
	t.Run("single cte", func(t *testing.T) {
		active := query.NewSelectBuilder(sqliteRenderer(t), "users").Columns("id").Where(query.Equal("active", true))
		q := query.NewSelectBuilder(sqliteRenderer(t), "active_users").Columns("id").With("active_users", active)
		sql, args, err := q.Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("cte with column list", func(t *testing.T) {
		source := query.NewSelectBuilder(sqliteRenderer(t), "orders").
			Projections(query.Project(query.Column("user_id")), query.ProjectAs(query.Count(query.Column("id")), "n")).
			GroupBy(query.Column("user_id"))
		q := query.NewSelectBuilder(sqliteRenderer(t), "totals").Columns("user_id", "n").With("totals", source, "user_id", "n")
		sql, args, err := q.Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("recursive cte", func(t *testing.T) {
		seed := query.NewSelectBuilder(sqliteRenderer(t), "tree").Columns("id", "parent_id").Where(query.IsNull("parent_id"))
		q := query.NewSelectBuilder(sqliteRenderer(t), "descendants").Columns("id").WithRecursive("descendants", seed)
		sql, args, err := q.Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("namespaced cte source and join stay logical", func(t *testing.T) {
		...
		left := query.NewSelectBuilder(renderer, "users").Columns("id", "team_id")
		right := query.NewSelectBuilder(renderer, "teams").Columns("id")
		statementBuilder := query.NewSelectFromCTE(renderer, "active_users", "u").
			Projections(query.Project(query.QualifiedColumn("u", "id"))).
			Join(query.LeftJoinCTE("active_teams", "t", query.EqualExpressions(query.QualifiedColumn("u", "team_id"), query.QualifiedColumn("t", "id")))).
			With("active_users", left).With("active_teams", right)
		sql, args, err := statementBuilder.Build()
		assertSQL(t, sql, args, err,
...
func TestSelectBuildWithOffset(t *testing.T) {
	sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "events").Columns("id").
		Where(query.And(query.Equal("workspace_id", "workspace"), query.Equal("status", "open"))).
		BuildWithOffset(3)
	assertSQL(t, sql, args, err, `SELECT "id" FROM "events" WHERE ("workspace_id" = $4 AND "status" = $5)`, []any{"workspace", "open"})
	if _, _, err := query.NewSelectBuilder(postgresRenderer(t), "events").Columns("id").BuildWithOffset(-1); err == nil {
...
func TestSelectDerivedTable(t *testing.T) {
	t.Run("from subquery", func(t *testing.T) {
		inner := query.NewSelectBuilder(sqliteRenderer(t), "events").
			Projections(query.Project(query.Column("user_id")), query.ProjectAs(query.Count(query.Column("id")), "n")).
			GroupBy(query.Column("user_id"))
		q := query.NewSelectFromSubquery(sqliteRenderer(t), inner, "e").
	...
	t.Run("lateral subquery join", func(t *testing.T) {
		lateral := query.NewSelectBuilder(postgresRenderer(t), "orders").Columns("id").
			Where(query.EqualExpressions(query.QualifiedColumn("orders", "user_id"), query.QualifiedColumn("u", "id"))).
			Limit(1)
		on := query.EqualExpressions(query.QualifiedColumn("o", "id"), query.QualifiedColumn("o", "id"))
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "users").Alias("u").
			Projections(query.Project(query.QualifiedColumn("u", "id"))).
			Join(query.LeftJoinSubquery(lateral, "o", on).Lateral()).Build()
		assertSQL(t, sql, args, err,
...
func TestSelectGroupingAndDistinctOn(t *testing.T) {
	t.Run("distinct on", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "events").
			DistinctOn(query.Column("user_id")).
			Projections(query.Project(query.Column("user_id")), query.Project(query.Column("created_at"))).
			OrderBy(query.Ascending("user_id"), query.Descending("created_at")).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("rollup", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "sales").
			Projections(query.Project(query.Column("region")), query.ProjectAs(query.Sum(query.Column("amount")), "total")).
			GroupByRollup(query.Column("region"), query.Column("product")).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("cube", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "sales").
			Projections(query.ProjectAs(query.Sum(query.Column("amount")), "total")).
			GroupByCube(query.Column("region")).Build()
	...
	t.Run("grouping sets", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "sales").
			Projections(query.ProjectAs(query.Sum(query.Column("amount")), "total")).
			GroupByGroupingSets(query.Column("region"), query.Column("product")).Build()
		assertSQL(t, sql, args, err,
...
func TestJoinVariants(t *testing.T) {
	...
	t.Run("right join", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "a").Alias("a").Columns("id").
			Join(query.RightJoin("b", "b", on)).Build()
		assertSQL(t, sql, args, err, `SELECT "id" FROM "a" AS "a" RIGHT JOIN "b" AS "b" ON "a"."id" = "b"."a_id"`, nil)
	})
	t.Run("full join", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "a").Alias("a").Columns("id").
			Join(query.FullJoin("b", "b", on)).Build()
		assertSQL(t, sql, args, err, `SELECT "id" FROM "a" AS "a" FULL JOIN "b" AS "b" ON "a"."id" = "b"."a_id"`, nil)
	})
	t.Run("cross join", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "a").Alias("a").Columns("id").
			Join(query.CrossJoin("b", "b")).Build()
		assertSQL(t, sql, args, err, `SELECT "id" FROM "a" AS "a" CROSS JOIN "b" AS "b"`, nil)
	})
	t.Run("subquery join", func(t *testing.T) {
		sub := query.NewSelectBuilder(sqliteRenderer(t), "orders").
			Projections(query.Project(query.Column("user_id")), query.ProjectAs(query.Count(query.Column("id")), "n")).
			GroupBy(query.Column("user_id"))
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "users").Alias("u").
			Projections(query.Project(query.QualifiedColumn("u", "id"))).
			Join(query.InnerJoinSubquery(sub, "o", query.EqualExpressions(query.QualifiedColumn("u", "id"), query.QualifiedColumn("o", "user_id")))).Build()
		assertSQL(t, sql, args, err,
...
func TestSubqueryPredicates(t *testing.T) {
	t.Run("in subquery", func(t *testing.T) {
		sub := query.NewSelectBuilder(sqliteRenderer(t), "bans").Columns("user_id").Where(query.Equal("active", true))
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "users").Columns("id").
			Where(query.InSubquery("id", sub)).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("not in subquery", func(t *testing.T) {
		sub := query.NewSelectBuilder(sqliteRenderer(t), "bans").Columns("user_id").Where(query.Equal("active", true))
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "users").Columns("id").
			Where(query.NotInSubquery("id", sub)).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("scalar comparison subquery", func(t *testing.T) {
		avg := query.NewSelectBuilder(sqliteRenderer(t), "orders").Projections(query.Project(query.Avg(query.Column("total"))))
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "orders").Columns("id").
			Where(query.GreaterThanSubquery(query.Column("total"), avg)).Build()
		assertSQL(t, sql, args, err,
			`SELECT "id" FROM "orders" WHERE "total" > (SELECT AVG("total") FROM "orders")`,
			nil)
	})
	t.Run("exists subquery", func(t *testing.T) {
		sub := query.NewSelectBuilder(sqliteRenderer(t), "orders").Columns("id").
			Where(query.EqualExpressions(query.QualifiedColumn("orders", "user_id"), query.QualifiedColumn("u", "id")))
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "users").Alias("u").
			Projections(query.Project(query.QualifiedColumn("u", "id"))).
			Where(query.ExistsSubquery(sub)).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("not exists subquery", func(t *testing.T) {
		...
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "users").Alias("u").
			Projections(query.Project(query.QualifiedColumn("u", "id"))).
			Where(query.NotExistsSubquery(sub)).Build()
		assertSQL(t, sql, args, err,
...
func TestILikeDialects(t *testing.T) {
	t.Run("postgres native ilike", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "users").Columns("id").
			Where(query.ILike("email", "%@EXAMPLE.com")).Build()
	...
	t.Run("mysql emulated ilike", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(mysqlRenderer(t), "users").Columns("id").
			Where(query.ILike("email", "%@EXAMPLE.com")).Build()
	...
	t.Run("sqlite emulated not ilike", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "users").Columns("id").
			Where(query.NotILike("name", "admin%")).Build()
...
func TestNullsOrdering(t *testing.T) {
	t.Run("postgres nulls last", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "t").Columns("id").
			OrderBy(query.Ascending("score").NullsLast()).Build()
	...
	t.Run("sqlite nulls first", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "t").Columns("id").
			OrderBy(query.Descending("score").NullsFirst()).Build()
	...
	t.Run("mysql emulated nulls last", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(mysqlRenderer(t), "t").Columns("id").
			OrderBy(query.Ascending("score").NullsLast()).Build()
...
func TestWindowAndScalarExpressions(t *testing.T) {
	t.Run("row_number over partition order", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "events").Projections(
			query.Project(query.Column("id")),
			query.ProjectAs(query.Over(query.RowNumber(), query.Window().
				PartitionBy(query.Column("user_id")).
				OrderBy(query.Descending("created_at"))), "rn"),
		).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("sum over with frame", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "ledger").Projections(
			query.ProjectAs(query.Over(query.Sum(query.Column("amount")), query.Window().
	...
	t.Run("cast", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "t").Projections(
			query.ProjectAs(query.Cast(query.Column("created_at"), "date"), "d"),
		).Build()
		assertSQL(t, sql, args, err, `SELECT CAST("created_at" AS date) AS "d" FROM "t"`, nil)
	})
	t.Run("filtered aggregate", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(postgresRenderer(t), "orders").Projections(
	...
	t.Run("generic func and arithmetic", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "t").Projections(
			query.ProjectAs(query.Func("ROUND", query.Divide(query.Column("a"), query.Column("b")), query.Value(2)), "r"),
			query.ProjectAs(query.Multiply(query.Column("x"), query.Value(10)), "scaled"),
		).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("star projection", func(t *testing.T) {
		sql, args, err := query.NewSelectBuilder(sqliteRenderer(t), "t").Projections(query.Project(query.Star())).Build()
...
func TestReturningClauses(t *testing.T) {
	...
	t.Run("update returning star", func(t *testing.T) {
		sql, args, err := query.NewUpdateBuilder(postgresRenderer(t), "users").
			Set("name", "Ann").Where(query.Equal("id", "u1")).Returning("*").Build()
		assertSQL(t, sql, args, err,
			`UPDATE "users" SET "name" = $1 WHERE "id" = $2 RETURNING *`,
			[]any{"Ann", "u1"})
	})
	t.Run("delete returning", func(t *testing.T) {
		sql, args, err := query.NewDeleteBuilder(postgresRenderer(t), "sessions").
			Where(query.Equal("id", "s1")).Returning("id").Build()
...
func TestUpsert(t *testing.T) {
	...
	t.Run("on conflict do update where", func(t *testing.T) {
		sql, args, err := query.NewInsertBuilder(postgresRenderer(t), "counters").
			Columns("id", "hits").Values("c1", 1).
			OnConflictDoUpdateWhere([]string{"id"}, query.GreaterThanExpression(query.Column("hits"), 0),
				query.AssignExpression("hits", query.Add(query.Column("hits"), query.Value(1))),
			).Build()
		assertSQL(t, sql, args, err,
	...
	t.Run("mysql on duplicate key update", func(t *testing.T) {
		sql, args, err := query.NewInsertBuilder(mysqlRenderer(t), "users").
			Columns("id", "email").Values("u1", "a@b.co").
			OnDuplicateKeyUpdate(query.Assign("email", "a@b.co")).Build()
		assertSQL(t, sql, args, err,
...
func TestAdvancedErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"subquery from without alias", func() (string, []any, error) {
			inner := query.NewSelectBuilder(r, "t").Columns("id")
			return query.NewSelectFromSubquery(r, inner, "").Columns("id").Build()
		}},
		{"cross join missing table", func() (string, []any, error) {
			return query.NewSelectBuilder(r, "a").Columns("id").Join(query.CrossJoin("", "b")).Build()
		}},
		{"in subquery nil query", func() (string, []any, error) {
			return query.NewSelectBuilder(r, "t").Columns("id").Where(query.InSubquery("id", nil)).Build()
		}},
		{"insert values and select", func() (string, []any, error) {
			source := query.NewSelectBuilder(r, "s").Columns("id")
			return query.NewInsertBuilder(r, "t").Columns("id").Values(1).FromSelect(source).Build()
		}},
		{"on duplicate key on non-mysql", func() (string, []any, error) {
			return query.NewInsertBuilder(r, "t").Columns("id").Values(1).
				OnDuplicateKeyUpdate(query.Assign("id", 2)).Build()
		}},
		...
		{"cast without type", func() (string, []any, error) {
			return query.NewSelectBuilder(r, "t").Projections(query.Project(query.Cast(query.Column("a"), ""))).Build()
		}},
	}
...
func TestBuilderRejectsRawSQLInjectionSurfaces(t *testing.T) {
	...
	tests := []struct {
		...
		{"function", func() (string, []any, error) {
			return query.NewSelectBuilder(renderer, "records").Projections(query.Project(query.Func("COUNT); DROP TABLE users;--", query.Column("id")))).Build()
		}},
		{"cast", func() (string, []any, error) {
			return query.NewSelectBuilder(renderer, "records").Projections(query.Project(query.Cast(query.Column("id"), "TEXT); DROP TABLE users;--"))).Build()
		}},
		{"window frame", func() (string, []any, error) {
			return query.NewSelectBuilder(renderer, "records").Projections(query.Project(query.Over(query.RowNumber(), query.Window().Frame("ROWS CURRENT ROW); DROP TABLE users;--")))).Build()
		}},
		{"row lock", func() (string, []any, error) {
			return query.NewSelectBuilder(renderer, "records").Columns("id").ForUpdate("NOWAIT; DROP TABLE users").Build()
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
