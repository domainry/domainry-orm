	...
	tests := []struct {
	...
	}{
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Columns("id", "email").Build()
			},
		...
		},
		{
			name: "distinct",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Distinct().Columns("country").Build()
			},
		...
		},
		{
			name: "table alias",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Alias("u").Columns("id").Build()
			},
		...
		},
		{
			name: "where equality",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Columns("id").Where(builder.Equal("id", 7)).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Columns("id").Where(builder.And(
					builder.GreaterThan("total", 100),
					builder.LessThanOrEqual("total", 500),
					builder.NotEqual("status", "void"),
				)).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "metrics").Columns("id").Where(builder.And(
					builder.LessThan("a", 1),
					builder.GreaterThanOrEqual("b", 2),
				)).Build()
			},
		...
		},
		{
			name: "nested and/or",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "records").Columns("id").Where(builder.Or(
					builder.And(builder.Equal("a", 1), builder.Equal("b", 2)),
					builder.Equal("c", 3),
				)).Build()
			},
		...
		},
		{
			name: "in and not in",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "records").Columns("id").Where(builder.And(
					builder.In("status", "a", "b"),
					builder.NotIn("kind", "x"),
				)).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "events").Columns("id").Where(builder.And(
					builder.Between("age", 18, 65),
					builder.NotBetween("score", 0, 10),
				)).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Columns("id").Where(builder.And(
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Projections(
					builder.Project(builder.Column("country")),
					builder.ProjectAs(builder.Count(builder.Column("id")), "total"),
					builder.ProjectAs(builder.Sum(builder.Column("amount")), "revenue"),
				).GroupBy(builder.Column("country")).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Projections(
					builder.ProjectAs(builder.Coalesce(builder.Column("nickname"), builder.Value("anon")), "display"),
				).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Projections(
					builder.ProjectAs(builder.Subtract(builder.Column("gross"), builder.Column("tax")), "net"),
				).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Projections(
					builder.Project(builder.Column("country")),
					builder.ProjectAs(builder.Count(builder.Column("id")), "total"),
				).GroupBy(builder.Column("country")).Having(
					builder.GreaterThanExpression(builder.Count(builder.Column("id")), 10),
				).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Columns("id").OrderBy(
					builder.Ascending("name"),
					builder.Descending("created_at"),
				).Build()
			},
		...
		},
		{
			name: "limit only",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Columns("id").Limit(10).Build()
			},
		...
		},
		{
			name: "offset only",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "users").Columns("id").Offset(5).Build()
			},
		...
		},
		{
			name: "inner join",
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Alias("o").Projections(
					builder.Project(builder.QualifiedColumn("o", "id")),
					builder.Project(builder.QualifiedColumn("u", "email")),
				).Join(
					builder.InnerJoin("users", "u", builder.EqualExpressions(
						builder.QualifiedColumn("o", "user_id"),
						builder.QualifiedColumn("u", "id"),
					)),
				).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Alias("o").Columns("id").Join(
					builder.LeftJoin("users", "u", builder.EqualExpressions(
						builder.QualifiedColumn("o", "user_id"),
						builder.QualifiedColumn("u", "id"),
					)),
				).Where(builder.IsNull("u")).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Alias("o").Distinct().Projections(
					builder.Project(builder.QualifiedColumn("o", "country")),
					builder.ProjectAs(builder.Sum(builder.QualifiedColumn("o", "amount")), "revenue"),
				).Join(
					builder.InnerJoin("users", "u", builder.EqualExpressions(
						builder.QualifiedColumn("o", "user_id"),
						builder.QualifiedColumn("u", "id"),
					...
					builder.EqualValue(builder.QualifiedColumn("o", "status"), "paid"),
					builder.InExpression(builder.QualifiedColumn("o", "country"), "US", "CA"),
				)).GroupBy(builder.QualifiedColumn("o", "country")).Having(
					builder.GreaterThanExpression(builder.Sum(builder.QualifiedColumn("o", "amount")), 1000),
				).OrderBy(builder.Descending("revenue")).Limit(20).Offset(40).Build()
			},
		...
		},
		{
			...
			build: func(r dialect.Renderer) (string, []any, error) {
				return builder.NewSelectBuilder(r, "orders").Columns("id").Where(
					builder.InExpression(builder.QualifiedColumn("o", "status"), "paid", "shipped"),
				).Build()
			},
...
func TestSelectPostgresPlaceholders(t *testing.T) {
	sql, args, err := builder.NewSelectBuilder(postgresRenderer(t), "users").Columns("id").Where(builder.And(
		builder.Equal("country", "US"),
		builder.In("status", "a", "b"),
		builder.Between("age", 18, 65),
	)).Limit(10).Build()
...
func TestInsertSQLVariants(t *testing.T) {
	t.Run("single row", func(t *testing.T) {
		sql, args, err := builder.NewInsertBuilder(sqliteRenderer(t), "users").
...
func TestUpdateSQLVariants(t *testing.T) {
	t.Run("single assignment", func(t *testing.T) {
		sql, args, err := builder.NewUpdateBuilder(sqliteRenderer(t), "users").
			Set("email", "new@b.co").Where(builder.Equal("id", "u1")).Build()
	...
	t.Run("multiple assignments", func(t *testing.T) {
		sql, args, err := builder.NewUpdateBuilder(sqliteRenderer(t), "users").
			Set("email", "new@b.co").
			Set("name", "Ann").
			Where(builder.Equal("id", "u1")).Build()
		assertSQL(t, sql, args, err,
			`UPDATE "users" SET "email" = ?, "name" = ? WHERE "id" = ?`,
			[]any{"new@b.co", "Ann", "u1"})
	})
	t.Run("expression assignment increment", func(t *testing.T) {
		sql, args, err := builder.NewUpdateBuilder(sqliteRenderer(t), "counters").
			SetExpression("hits", builder.Add(builder.Column("hits"), builder.Value(1))).
			Where(builder.Equal("id", "c1")).Build()
		assertSQL(t, sql, args, err,
			`UPDATE "counters" SET "hits" = "hits" + ? WHERE "id" = ?`,
			[]any{1, "c1"})
	})
	t.Run("expression assignment decrement", func(t *testing.T) {
		sql, args, err := builder.NewUpdateBuilder(sqliteRenderer(t), "inventory").
			SetExpression("stock", builder.Subtract(builder.Column("stock"), builder.Value(3))).
			Where(builder.Equal("sku", "abc")).Build()
		assertSQL(t, sql, args, err,
			`UPDATE "inventory" SET "stock" = "stock" - ? WHERE "sku" = ?`,
			[]any{3, "abc"})
	})
	t.Run("mixed value and expression with complex predicate", func(t *testing.T) {
		sql, args, err := builder.NewUpdateBuilder(sqliteRenderer(t), "records").
			Set("name", "Updated").
			SetExpression("revision", builder.Add(builder.Column("revision"), builder.Value(1))).
			Where(builder.And(builder.Equal("workspace_id", "w1"), builder.In("status", "a", "b"))).Build()
		assertSQL(t, sql, args, err,
			`UPDATE "records" SET "name" = ?, "revision" = "revision" + ? WHERE ("workspace_id" = ? AND "status" IN (?, ?))`,
			[]any{"Updated", 1, "w1", "a", "b"})
	})
	t.Run("postgres placeholders", func(t *testing.T) {
		sql, args, err := builder.NewUpdateBuilder(postgresRenderer(t), "records").
			Set("name", "Updated").
			SetExpression("revision", builder.Add(builder.Column("revision"), builder.Value(1))).
			Where(builder.Equal("id", "one")).Build()
		assertSQL(t, sql, args, err,
...
func TestDeleteSQLVariants(t *testing.T) {
	t.Run("equality", func(t *testing.T) {
		sql, args, err := builder.NewDeleteBuilder(sqliteRenderer(t), "users").
			Where(builder.Equal("id", "u1")).Build()
		assertSQL(t, sql, args, err, `DELETE FROM "users" WHERE "id" = ?`, []any{"u1"})
	})
	t.Run("compound predicate", func(t *testing.T) {
		sql, args, err := builder.NewDeleteBuilder(sqliteRenderer(t), "sessions").
			Where(builder.And(builder.LessThan("expires_at", 1000), builder.IsNotNull("user_id"))).Build()
		assertSQL(t, sql, args, err,
			`DELETE FROM "sessions" WHERE ("expires_at" < ? AND "user_id" IS NOT NULL)`,
			[]any{1000})
	})
	t.Run("in predicate", func(t *testing.T) {
		sql, args, err := builder.NewDeleteBuilder(sqliteRenderer(t), "records").
			Where(builder.In("id", "a", "b", "c")).Build()
		assertSQL(t, sql, args, err, `DELETE FROM "records" WHERE "id" IN (?, ?, ?)`, []any{"a", "b", "c"})
	})
	t.Run("postgres placeholders", func(t *testing.T) {
		sql, args, err := builder.NewDeleteBuilder(postgresRenderer(t), "records").
			Where(builder.And(builder.Equal("workspace_id", "w1"), builder.Equal("id", "one"))).Build()
		assertSQL(t, sql, args, err,
...
func TestBuildersHonorNamespace(t *testing.T) {
	...
	selectSQL, _, err := builder.NewSelectBuilder(renderer, "users").Columns("id").Build()
	assertSQL(t, selectSQL, nil, err, `SELECT "id" FROM "app"."t_users"`, nil)

	insertSQL, insertArgs, err := builder.NewInsertBuilder(renderer, "users").Columns("id").Values("u1").Build()
	assertSQL(t, insertSQL, insertArgs, err, `INSERT INTO "app"."t_users" ("id") VALUES ($1)`, []any{"u1"})

	updateSQL, updateArgs, err := builder.NewUpdateBuilder(renderer, "users").Set("id", "u2").Where(builder.Equal("id", "u1")).Build()
	assertSQL(t, updateSQL, updateArgs, err, `UPDATE "app"."t_users" SET "id" = $1 WHERE "id" = $2`, []any{"u2", "u1"})

	deleteSQL, deleteArgs, err := builder.NewDeleteBuilder(renderer, "users").Where(builder.Equal("id", "u1")).Build()
...
func TestBuilderErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"select without columns", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Build()
		}},
		{"select empty table", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "   ").Columns("id").Build()
		}},
		{"select negative limit", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Columns("id").Limit(-1).Build()
		}},
		{"select negative offset", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Columns("id").Offset(-1).Build()
		}},
		{"select empty in", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Columns("id").Where(builder.In("status")).Build()
		}},
		{"select empty compound", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Columns("id").Where(builder.And()).Build()
		}},
		{"select nil group expression", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Columns("id").GroupBy(nil).Build()
		}},
		{"select join without predicate", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "orders").Columns("id").Join(builder.InnerJoin("users", "u", nil)).Build()
		}},
		{"projection nil expression", func() (string, []any, error) {
			return builder.NewSelectBuilder(r, "users").Projections(builder.Project(nil)).Build()
		}},
		{"insert without columns", func() (string, []any, error) {
			return builder.NewInsertBuilder(r, "users").Values("u1").Build()
		}},
		{"insert without values", func() (string, []any, error) {
			return builder.NewInsertBuilder(r, "users").Columns("id").Build()
		}},
		{"insert row arity mismatch", func() (string, []any, error) {
			return builder.NewInsertBuilder(r, "users").Columns("id", "email").Values("only-id").Build()
		}},
		{"update without assignments", func() (string, []any, error) {
			return builder.NewUpdateBuilder(r, "users").Where(builder.Equal("id", "u1")).Build()
		}},
		{"update without predicate", func() (string, []any, error) {
			return builder.NewUpdateBuilder(r, "users").Set("name", "x").Build()
		}},
		{"update blank column", func() (string, []any, error) {
			return builder.NewUpdateBuilder(r, "users").Set("  ", "x").Where(builder.Equal("id", "u1")).Build()
		}},
		{"delete without predicate", func() (string, []any, error) {
			return builder.NewDeleteBuilder(r, "users").Build()
		}},
	}
